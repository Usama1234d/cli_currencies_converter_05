...
while (continueConverting) {
    ...
    const answer = await inquirer.prompt([
        ...
        },
        {
            type: "number",
            name: "amount",
            message: chalk.bgBlueBright("Please enter the amount you wish to convert"),
        },
    ]);
    ...
    if (from && to && amount) {
        ...
        console.log(chalk.redBright(`your conversion from ${from} to ${to} is ${result.toFixed(2)}`));
    }
    else {
        console.log(chalk.bgRedBright("invalid conversion. Make sure you have entered correct currency"));
    }
    // Ask if the user wants to continue
    const continueOperation = await inquirer.prompt({
        type: "confirm",
        message: chalk.bgMagentaBright("Would you like to perform another conversion"),
        name: "continue",
        default: true,
    });
    // Updating
